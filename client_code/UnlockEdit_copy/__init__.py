from ._anvil_designer import UnlockEdit_copyTemplate
from anvil import *
import anvil.server
import anvil.users
...
class UnlockEdit_copy(UnlockEdit_copyTemplate):
  def __init__(self, **properties):
    # Set Form properties and Data Bindings.
    self.init_components(**properties)

    self.item_type_drop_down.items = [
      "Head",
      "Body",
      "Feet",
      "One Hand",
  ...
  def item_change(self):
    ...
    if not self.item_selected:
      alert(
        f"{self.item_number_text_box.text} is not an item! Returning to item {self.previous_item}"
      )
  ...
  def building_change(self):
    ...
    if self.building_selected["CurrentLevel"] > 0:
      self.building = app_tables.buildings.get(
        Name=self.building_selected["Name"],
        Level=self.building_selected["CurrentLevel"],
      )
    ...
    self.building_level_drop_down.items = [
      str(level) for level in range(self.building_selected["Max Level"] + 1)
    ]
    self.building_level_drop_down.selected_value = str(
      self.building_selected["CurrentLevel"]
    )
  ...
  def building_level_drop_down_change(self, **event_args):
    self.building_selected["CurrentLevel"] = int(
      self.building_level_drop_down.selected_value
    )
    self.building_selected.update()
    self.building_change()
  ...
  def scenario_change(self):
    ...
    sticker_media = URLMedia(
      f"https://github.com/any2cards/frosthaven/blob/master/images/art/frosthaven/stickers/individual/location-stickers/fh-{sticker_number:03d}-{sticker_name}.png?raw=true"
    )
