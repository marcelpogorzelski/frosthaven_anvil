from ._anvil_designer import FinishScenario_copyTemplate
from anvil import *
import anvil.server
import anvil.users
...
class FinishScenario_copy(FinishScenario_copyTemplate):
  ...
  def set_coin_value(self):
    gold_conversion = app_tables.scenario_info.get(
      Level=self.party_level_text_box.text
    )["Gold Conversion"]
    self.coin_value_text_box.text = gold_conversion

  def set_bonus_experience(self):
    bonus_experience = app_tables.scenario_info.get(
      Level=self.party_level_text_box.text
    )["Bonus Experience"]
